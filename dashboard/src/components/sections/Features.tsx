...
const FEATURES = [
	{ icon: Cpu, title: "Multi-device support", desc: "Link as many ESP32 boards as you need to a single account using a shared API key. Each board shows up independently in your dashboard." },
	{
		...
		desc: "Commands reach your ESP32 instantly over a persistent WebSocket connection. No polling, no delays - toggle a relay and it switches in milliseconds.",
	},
	{
		icon: ToggleRight,
		title: "Up to 8 relays per board",
		desc: "Assign GPIO pins, give each relay a friendly name and icon, and control them with a single tap. Offline state syncs automatically on reconnect.",
	},
...
export default function FeaturesSection() {
	...
	return (
		<section id="features" ref={sectionRef} className="py-24 px-6 md:px-[60px] max-w-[1200px] mx-auto">
			<div className="text-center mb-16 reveal">
				...
				<p className="text-[16px] max-w-[520px] mx-auto mt-5 leading-relaxed" style={{ color: "#7a9080" }}>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
				{FEATURES.map(({ icon: Icon, title, desc }) => (
					<div key={title} className="reveal land-card group p-7">
						<div className="w-10 h-10 rounded-xl flex items-center justify-center mb-5 transition-colors" style={{ background: "rgba(18,201,132,0.1)" }}>
							<Icon className="w-5 h-5" style={{ color: "#12c984" }} />
						</div>
						<h3 className="font-['Sora'] font-bold text-[17px] mb-2" style={{ color: "#e8f0ea" }}>
							{title}
						</h3>
