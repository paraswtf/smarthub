...
export default function DashboardSidebar({ user }: Props) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t space-y-1" style={{ borderColor: "hsl(var(--sidebar-border))" }}>
        ...
        <button
          ...
          className="dash-nav-item w-full text-left hover:text-destructive"
        >
  ...
  return (
    <>
      ...
      <button
        className="lg:hidden fixed top-4 left-4 z-50 w-9 h-9 rounded-lg flex items-center justify-center bg-card border border-border shadow-sm"
      ...
      <aside
        className={cn(
          "lg:hidden fixed top-0 left-0 z-50 h-full w-64 dash-sidebar transition-transform duration-300",
          mobileOpen ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <button
          className="absolute top-4 right-4 text-sidebar-muted hover:text-sidebar-foreground transition-colors"
          onClick={() => setMobileOpen(false)}
        >
          <X className="w-5 h-5" />
        </button>
