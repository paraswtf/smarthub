...
export const homeRouter = createTRPCRouter({
	...
	/** Create a new home */
	create: protectedProcedure.input(z.object({ name: z.string().min(1).max(60) })).mutation(async ({ ctx, input }) => {
	...
	/** Rename a home */
	update: protectedProcedure.input(z.object({ id: z.string(), name: z.string().min(1).max(60) })).mutation(async ({ ctx, input }) => {
	...
	assignDevice: protectedProcedure.input(z.object({ deviceId: z.string(), homeId: z.string().nullable() })).mutation(async ({ ctx, input }) => {
