...
export const sharingRouter = createTRPCRouter({
	...
	shareHome: protectedProcedure.input(z.object({ homeId: z.string(), email: z.string().email() })).mutation(async ({ ctx, input }) => {
	...
	/** Remove a home share */
	unshareHome: protectedProcedure.input(z.object({ homeId: z.string(), userId: z.string() })).mutation(async ({ ctx, input }) => {
		const home = await ctx.db.home.findFirst({
			where: { id: input.homeId, ownerId: ctx.session.user.id },
		});
		if (!home) throw new TRPCError({ code: "FORBIDDEN" });
	...
	listHomeShares: protectedProcedure.input(z.object({ homeId: z.string() })).query(async ({ ctx, input }) => {
		const home = await ctx.db.home.findFirst({
			where: { id: input.homeId, ownerId: ctx.session.user.id },
		});
		if (!home) throw new TRPCError({ code: "FORBIDDEN" });
	...
	shareRoom: protectedProcedure.input(z.object({ roomId: z.string(), email: z.string().email() })).mutation(async ({ ctx, input }) => {
	...
	/** Remove a room share */
	unshareRoom: protectedProcedure.input(z.object({ roomId: z.string(), userId: z.string() })).mutation(async ({ ctx, input }) => {
		const room = await ctx.db.room.findFirst({
			where: { id: input.roomId },
			include: { home: { select: { ownerId: true } } },
		});
	...
	listRoomShares: protectedProcedure.input(z.object({ roomId: z.string() })).query(async ({ ctx, input }) => {
		const room = await ctx.db.room.findFirst({
			where: { id: input.roomId },
			include: { home: { select: { ownerId: true } } },
		});
	...
	shareRelay: protectedProcedure.input(z.object({ relayId: z.string(), email: z.string().email() })).mutation(async ({ ctx, input }) => {
	...
	unshareRelay: protectedProcedure.input(z.object({ relayId: z.string(), userId: z.string() })).mutation(async ({ ctx, input }) => {
		const relay = await ctx.db.relay.findFirst({
			where: { id: input.relayId },
			include: { device: { select: { apiKey: { select: { userId: true } } } } },
		});
	...
	listRelayShares: protectedProcedure.input(z.object({ relayId: z.string() })).query(async ({ ctx, input }) => {
		const relay = await ctx.db.relay.findFirst({
			where: { id: input.relayId },
			include: { device: { select: { apiKey: { select: { userId: true } } } } },
		});
