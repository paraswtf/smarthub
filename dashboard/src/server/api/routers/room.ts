...
export const roomRouter = createTRPCRouter({
	...
	create: protectedProcedure
		.input(z.object({ homeId: z.string(), name: z.string().min(1).max(60) }))
	...
	assignRelay: protectedProcedure
		.input(z.object({ relayId: z.string(), roomId: z.string() }))
		.mutation(async ({ ctx, input }) => {
	...
	unassignRelay: protectedProcedure
	...
	unassignedRelays: protectedProcedure
		.input(z.object({ homeId: z.string() }))
		.query(async ({ ctx, input }) => {
			const home = await ctx.db.home.findFirst({
				where: { id: input.homeId, ownerId: ctx.session.user.id }
